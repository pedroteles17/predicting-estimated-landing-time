...
def calculate_expected_arrival(row):
    return row["estimated_departure"] + pd.Timedelta(seconds=row["origin_destiny_encode"])

#%%
train = pd.read_parquet("data/modelling/train_df.parquet")\
    .assign(
        origin_destiny = lambda x: x["origem"] + "_" + x["destino"]
    )

target_encoding_origin_destiny = train.groupby("origin_destiny")["seconds_flying"].mean()
...
train = train\
    .assign(
        origin_destiny_encode = lambda x: x["origin_destiny"].map(target_encoding_origin_destiny),
        expected_arrival = lambda x: x.apply(calculate_expected_arrival, axis=1)
    )\
    .drop([
        "flightid", "origem", "destino", "origin_destiny",
        "dt_dep", "path", "unique_metar", "time", "image_date"
    ], axis=1)
...
train[(train["expected_arrival"] > train["expected_arrival"][285087]) & (train["expected_arrival"] < train["expected_arrival"][285087] + pd.Timedelta(minutes=15))]
